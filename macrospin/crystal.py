...
def angle_between_directions(start, normal, directions, rotation='right', decimal_places=5):
    ...
    angles = np.arccos(np.round(directions.dot(start), decimal_places))*180/np.pi
    if rotation == 'right':
...
def in_plane_indices(normal, magnitude=1, decimal_places=5):
    ...
    return indices[np.where(np.round(normalize(indices).dot(normal), decimal_places)==0)]
    
def in_plane_indices_and_angles(start, normal, rotation='right', magnitude=1, decimal_places=5):
    ...
    """
    in_plane_indices = in_plane_indices(normal, magnitude, decimal_places)
    angles = angle_between_directions(start, normal, normalize(in_plane_indices))
    mask = angles.argsort()
